...
class OpenAICompatibleEngine(InferenceEngine):
    """Базовый движок для доступа к OpenAI-совместимому API."""
    ...
    def _fix_tool_call_arguments(msg_dicts: list) -> list:
        """Проверяет, что tool_call аргументы - словари, а не JSON-строки.

        OpenAI-совместимые сервера (vLLM, SGLang, llama.cpp, etc.) ожидают
        tool_call аргументы как JSON объекты.  messages_to_dicts может
        сериализовать их как строки, что приведет к  ошибке 400.
        """
        for md in msg_dicts:
